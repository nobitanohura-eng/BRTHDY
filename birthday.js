...
function drawPetals(t, dt){
  for (let i = petals.length - 1; i >= 0; i--){
    ...
    // Gentle wind breeze push from mouse cursor
    if (mouseX > 0 && mouseY > 0) {
      ...
      if (dist < 100) {
        ...
        p.x += Math.cos(angle) * force * 160 * dt;
        p.y += Math.sin(angle) * force * 160 * dt;
        p.vx += Math.cos(angle) * force * 80 * dt;
      }
    }
...
function showWish(on){
  ...
  videoCard.classList.toggle('is-in', on);
  if (creditBadge) {
    creditBadge.classList.toggle('is-in', on);
  }
  if (on) {
    bdayVideo.play().catch(err => console.log("Video autoplay failed:", err));
  } else {
    bdayVideo.pause();
    ...
    const wishLetter = $('wishLetter');
    const modalInteraction = $('modalInteraction');
    const candleFlame = $('candleFlame');
    if (wishModal) wishModal.classList.remove('is-open');
    if (wishLetter) wishLetter.classList.remove('is-revealed');
    if (modalInteraction) {
      modalInteraction.style.display = 'block';
      modalInteraction.style.opacity = '1';
    }
    if (candleFlame) {
...
/* ============================================================
   INTERACTIVE CLICK SPARKLES (Magic Touch)
   ============================================================ */
function spawnSparkles(x, y) {
  const colors = ['#ff6f97', '#ffcf6a', '#ffd36a', '#e23b67', '#ffffff'];
  ...
  for (let i = 0; i < count; i++) {
    ...
    const size = isHeart ? rand(12, 20) : rand(4, 9);
    ...
    if (isHeart) {
      el.innerHTML = `<svg viewBox="0 0 24 22" width="100%" height="100%"><path d="M12 20C5.5 15 1.5 11.4 1.5 6.9 1.5 3.6 4 1.5 7 1.5c2 0 3.4 1.1 5 3 1.6-1.9 3-3 5-3 3 0 5.5 2.1 5.5 5.4C23.5 11.4 19.5 15 12 20Z" fill="${pick(colors)}"/></svg>`;
    } else {
      el.style.borderRadius = '50%';
      el.style.background = `radial-gradient(circle, #fff, ${pick(colors)} 70%)`;
    }
    
    container.appendChild(el);
    
    const angle = rand(0, Math.PI * 2);
    const distance = rand(30, 100);
...
window.addEventListener('pointerdown', (e) => {
  if (e.target.tagName === 'BUTTON' || e.target.tagName === 'VIDEO' || e.target.closest('#archery') || e.target.closest('.replay') || e.target.closest('.wish-modal__content')) return;
  spawnSparkles(e.clientX, e.clientY);
});

/* Pointer trackers for canvas wind effect */
...
/* Make a Wish Modal Event Listeners */
...
const modalCloseXBtn = $('modalCloseXBtn');
...
if (candleFlame) {
  candleFlame.addEventListener('click', () => {
    // Blow out candle: fade out flame
    gsap.to(candleFlame, {
      scale: 0,
      opacity: 0,
      duration: 0.3,
    ...
    // Sparkle explosion at flame position
    const r = candleFlame.getBoundingClientRect();
    spawnSparkles(r.left + r.width / 2, r.top + r.height / 2);
    
    // Fade out cake and instructions, then show letter
    gsap.to(modalInteraction, {
      opacity: 0,
      y: -10,
      duration: 0.5,
      delay: 0.2,
